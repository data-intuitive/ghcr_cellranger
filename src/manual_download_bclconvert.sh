...
for tag in 3.10.12 4.0.5 4.1.5 4.1.7 4.1.23 4.2.4 4.2.7; do
    ...
    if [ ! -f "$bclconvert_rpm" ]; then
        ...
        viash run src/bclconvert/download_bclconvert/config.vsh.yaml -- --tag $tag --email "$ILLUMINA_ACCOUNT" --password "$ILLUMINA_PASS" --output "$bclconvert_rpm" --gh_token "$GH_TOKEN"
    fi
